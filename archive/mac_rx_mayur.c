                          ...
                          char        *mac)
{
    ...
    ptr = db;
    ptr = ptr+k+15;  // this is the last mac byte...TODO: arrrgh forgot their are MSB aligned..!!
    for (k=0; k<16; k++) {
...

{
    ...
    printf("------------------------------------------------------------------\n");
    printf("Retrieving Frame counter value (32bit register)...\n");
    ptr = db;                                                                                          // reset pointer to first address of database

    #if 1   //TBR Mayur for making FC 0 ; testing (if u want frame count from embedded data then enable this part of code)
...

{
    ...
    const struct option l_option[]={
             ...
             { "live_data",  required_argument, NULL, 'L'},   
             { "embedded_data",   required_argument, NULL, 'E'}, 			 
    };
    ...
    int dataformat;
    int compandformat = 0;
    unsigned int sft_act_compand;
    unsigned int padding_embed_compand;
    int paddingreg;
    ...
    do {
              ...
              switch(n_opt){
                      case 'h':
							  ...
							  printf("\t-L (--live_data):\t\tFor Image with Live data & embedded data, set flag to 1(default) else set it to 0(for test pattern, embedded data ('-E') need to set either 0 or 1)\n");
							  printf("\t-E (--embedded_data):\t\tFor Image with embedded data aplicable only for test pattern image (set '-L' to 0), set flag to 1(default) else set it to 0 (w/o embedded data)\n");
                      ...
                      case 'x':
                              ...
                              break;
					  case 'L':
							  live_data = atoi(optarg);
							  if (live_data)
                                 printf("\tImage is with live data:\t%d\n",live_data);
							  else
							     printf("\tImage is with test pattern\t\n");
                              break;
					  case 'E':
							  embedded_data = atoi(optarg);
							  if (embedded_data)
                                 printf("\tImage is with embedded data:\t%d\n",embedded_data);
							  else
							     printf("\tImage is with out embedded data\t\n");
							  //live_data = 0;
                              break;
                      case -1:
                              break;
        ...
        while ((read = getline(&line, &len, fp)) > 15) {      // this should then finish when seeing CSV_FOOTER...
            ...
            {                
               ...
               // printf("prt after  strtok of NULL: %s, n:%d\n-------\n",ptr,n);
                ...
                if (live_data) { // Ashu: for Live video               
	                ...
	                if (embedded_data) { //with embedded data
    ...
    printf("------------------------------------------------------------------\n");
    char ivplusfc[24];
    if(!cmac) get_framecount_plus_iv(index,dbpix,piv32_0,piv32_1,piv32_2,iv,ivplusfc,ignorefc,sft_top_embed);
    /* ---------------------------------------------------------------------------------------------------------------------------------------------------------- */
    ...
    for (k=0; k<tmpindex;k++) {                 
        if (pixptr->row < macrow) {     // IGNORE last 2 MAC rows (not part of calculations)
            ...
            if ((packmode == mode0_8)||(packmode == mode1_8)) {          //no paccking required for eiher modes
				...
				if (k<10 || (k > 8207 && k < 8300))/*(k < 15)*/ printf(" tmpdata=%06x", tmpdata);
                sprintf((char*)tmpdata,"%01x",c&0xF);
				if (k<10 || (k > 8207 && k < 8300))/*(k < 15)*/ printf("\ttmpdata_c=%01x", tmpdata);
                tmpdata = tmpdata+1;
				
                sprintf((char*)tmpdata,"%02x",b);
                if (k<10 || (k > 8207 && k < 8300))/*(k < 15)*/ printf("\ttmpdata_b=%02x", tmpdata);
				tmpdata = tmpdata+2;
				
                sprintf((char*)tmpdata,"%02x",a);
                if (k<10 || (k > 8207 && k < 8300))/*(k < 15)*/ printf("\ttmpdata_a=%02x\n", tmpdata);
				tmpdata = tmpdata+2;
                ...
                if(sample == 6) {                        //every 6 pixels padding is added
                    sample = 0;
                    sprintf((char*)tmpdata,"%02x",paddingreg);  // 8 bit padding pushed-in
					if (k<10 || (k > 8207 && k < 8300))/*(k < 15)*/ printf("\tpaddingreg = %02x, tmpdata final=%02x\n", paddingreg, tmpdata);
                    tmpdata = tmpdata+2;
                }
    ...
    fprintf(ofp, "%s\n", data2tag_string);
    fclose(ofp);
    printf("\t\t-> tsize data2tag_string: %zu\n",strlen((const char*)data2tag_string));
    ...
    /* ---------------------------------------------------------------------------------------------------------------------------------------------------------- */
    unsigned char iv_hexarr[strlen(ivplusfc)/2] ;
	int size_iv;
	if(cmac == 0) { //For GMAC only consider IV + Frame count ; if condition done by Mayur
		printf("\t-----------------------------------------------\n");
		printf("\tprocessing ivplusfc (%s) string (of length: %zu) to byte-array..\n",(const char*)ivplusfc,strlen((const char*)ivplusfc));
    ...
    if(cmac == 0) {    // GMAC
        ...
        printf ("\t->Cipher set to GCM AES 128\n");
		...
		//printf ("\tdata2tag_hexarr: %s, \tsize_data2tag: %d, \toutlen: %d\n", data2tag_hexarr, size_data2tag, outlen);
