...
class TestAccessClient():
    ...
    __TAS_REQUEST_GET_FRAME_MAC_STATS   = 7
    ...
    def get_frame_mac_stats(self):
        """Requests the mac values.
        """
        # Packing is: request, payload_size_bytes
        req = struct.pack("!LLL", self.__TAS_REQUEST_GET_FRAME_MAC_STATS, 4, 0)
        ...
        # Receive size
        resp = self.__client_socket.recv(4)
        (frame_cnt, ) = struct.unpack("!L", resp)

        TAS_MAC_STATS_SIZE = 17 # size of TAS_MAC_STATS_T struct in test_access_server
        bytes_to_recv = (frame_cnt * TAS_MAC_STATS_SIZE)

        # Receive stats - note .recv() may not return all the requested data in one call
        if self.__debug:
            print("Receiving {} frame stats...".format(bytes_to_recv))

        recv_data = self.__client_socket.recv(bytes_to_recv)
