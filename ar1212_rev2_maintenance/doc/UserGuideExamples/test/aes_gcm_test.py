...
def aes_gcm_test(crypto_helper):
    ...
    lastColumn = 0x5678
    columnSkip = 8
    frameCounter = 0xaabbccdd12345678
    ...
    ciphertext, mac = crypto_helper.gcm_encrypt(control_channel_key,
    ...
    with open("doc/UserGuideExamples/output/aes_gcm_ciphertext.bin", 'w+b') as cipher_file:
        cipher_file.write(ciphertext)
        cipher_file.write(mac)

def main():
    """main function"""
    print("======== Python AES-GCM test running ========")
    crypto_helper = CryptoHelper()

    aes_gcm_test(crypto_helper)
