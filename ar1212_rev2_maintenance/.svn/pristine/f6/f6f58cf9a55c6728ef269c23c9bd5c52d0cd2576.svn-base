...
/// \file
///
/// Example AES-CCM encryption support function definition
///
//***********************************************************************************
...
/// Encrypts supplied plaintext using AES-CCM
///
/// \param[in] pKey                 128-bit AES key
/// \param[in] pNonce               CCM nonce
/// \param[in] nonceSizeBytes       Size of the nonce
...
///
int Aes128CcmEncrypt(   uint8_t* const pKey,
                        uint8_t* const pNonce,
                        size_t const nonceSizeBytes,
                        ...
                        uint8_t* const tag)
{
    ...
    res = EVP_EncryptInit_ex(ctx, EVP_aes_128_ccm(), NULL, NULL, NULL);
    if (0 >= res)
    {
    ...
    res = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, nonceSizeBytes, NULL);
    if (0 >= res)
    {
        printLastError("EVP_CIPHER_CTX_ctrl (nonce len) failed");
        return -1;
    }

    // Set the 'L' length
    // CCM nonce is 15 - L
    res = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_CCM_SET_L, 15U - nonceSizeBytes, NULL);
    if (0 >= res)
    {
        printLastError("EVP_CIPHER_CTX_ctrl (L len) failed");
        return -1;
    }

    // Set the MAC length
    res = EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, CSS_MAC_SIZE_BYTES, NULL);
    if (0 >= res)
    {
        printLastError("EVP_CIPHER_CTX_ctrl (tag len) failed");
        return -1;
    }

    // Set the key and nonce
    res = EVP_EncryptInit_ex(ctx, NULL, NULL, pKey, pNonce);
    if (0 >= res)
    {
        printLastError("EVP_EncryptInit_ex(pKey, pNonce) failed");
        return -1;
    }

    // Provide the total plaintext length
    res = EVP_EncryptUpdate(ctx, NULL, &encryptSizeBytes, NULL, plaintextSizeBytes);
    if (0 >= res)
    {
        printLastError("EVP_EncryptUpdate(plaintext len) failed");
        return -1;
    }
    ...
    res = EVP_EncryptUpdate(ctx, ciphertext, &encryptSizeBytes, plaintext, plaintextSizeBytes);
    if (0 >= res)
    {
        printLastError("EVP_EncryptUpdate(plaintext) failed");
        return -1;
    }

    // Finalise the encryption
    ciphertextSizeBytes = encryptSizeBytes;
    ciphertextBlock = &ciphertext[ciphertextSizeBytes];
    ...
    if (0 >= res)
    {
        printLastError("EVP_CIPHER_CTX_ctrl(tag) failed");
        return -1;
    }
...
/// Decrypts supplied ciphertext using AES-CCM
///
/// \param[in] pKey                 128-bit AES key
/// \param[in] pNonce               CCM nonce
/// \param[in] nonceSizeBytes       Size of the nonce
                        ...
                        uint8_t* const plaintext)
{
    ...
    res = EVP_DecryptInit_ex(ctx, EVP_aes_128_ccm(), NULL, NULL, NULL);
    if (0 >= res)
    {
    ...
    // Set the key and nonce
    res = EVP_DecryptInit_ex(ctx, NULL, NULL, pKey, pNonce);
    if (0 >= res)
    {
        printLastError("EVP_DecryptInit_ex(pKey, pNonce) failed");
        return -1;
    }

    // Provide the total ciphertext length
    res = EVP_EncryptUpdate(ctx, NULL, &decryptSizeBytes, NULL, ciphertextSizeBytes);
    if (0 >= res)
    {
        printLastError("EVP_EncryptUpdate(ciphertext len) failed");
        return -1;
    }
    ...
    // Decrypt the ciphertext - returns 0 only if MAC is good
    ...
    plaintextSizeBytes = decryptSizeBytes;
...
/// Encrypts supplied Video Authentication configuration params using AES-CCM
///
                        ...
                        uint8_t* const mac)
{
    ...
    encryptedSizeBytes = Aes128CcmEncrypt(  (uint8_t* const)controlChannelKey,
...
/// Decrypts supplied configuration params using AES-CCM
///
                        ...
                        CssVideoAuthConfigParamsT* const configParams)
{
    ...
    decryptedSizeBytes = Aes128CcmDecrypt(  (uint8_t* const)controlChannelKey,
                                            (uint8_t* const)controlChannelNonce,
                                            sizeof(controlChannelNonce),
                                            encryptedParams,
                                            sizeof(CssVideoAuthConfigParamsT),
...
int CcmEncryptDecryptTest(void)
{
    CssVideoAuthConfigParamsT configParams;
    ...
    printf("\n\nAES-CCM encrypt/decrypt test running\n");
...
{
    printf("\n======== OpenSSL AES-CCM encrypt/decrypt demo running ========\n");

    CcmEncryptDecryptTest();
    DecryptConfigParamsFromFile("doc/UserGuideExamples/output/aes_ccm_ciphertext.bin");
